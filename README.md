...
- канонические листы `Преподаватели`, `Группы`, `Студенты`, `Нагрузка`, `Специальности`, `Учебные планы`, `Дисциплины`;
...
Три справочных листа добавляются только полным набором. Для совместимости поддерживается старый трёхлистовый файл без учебных планов; канонический семилистовый формат дополнительно включает:
...
- [x] Предпросмотр, сверка часов и атомарная активация семи разделов.
- [ ] Корпуса и аудитории из Excel.
...
- [ ] Технический Windows alpha EXE после импорта аудиторий: launcher и внутренний installer без решателя.
...
Проект находится на стадии `0.2 alpha`: импорт студентов и учебных планов готов, расчёт расписания ещё не реализован. Технический Windows alpha EXE запланирован после импорта аудиторий, а функциональный — после календаря и первого решателя. Решение по поставке описано в [ADR-001](docs/decisions/001-windows-distribution.md).
